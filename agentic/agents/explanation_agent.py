...
def explanation_agent(state):
    ...
    predicted = state.get("predicted_delay")       # ← proactive used predicted_delay_hours
    actual    = state.get("actual_delay")          # ← proactive used actual_delay_hours
    severity  = state.get("severity")
    stay_df   = state.get("stay_df")
    ...
    rr          = last_row.get("RR")
    spo2        = last_row.get("SPO2")
    trigger     = last_row.get("trigger_type", "Unknown")  # ← proactive used episode_type
...
Predicted expected lab delay: {predicted:.2f} hours
Actual lab delay: {round(actual, 2) if actual is not None else 'N/A'}
Severity classification: {severity}
